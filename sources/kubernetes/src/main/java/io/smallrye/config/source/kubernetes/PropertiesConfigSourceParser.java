...
public class PropertiesConfigSourceParser implements ConfigSourceParser {
    ...
    public ConfigSource parser(Path path) {
        try (InputStream stream = Files.newInputStream(path)) {
            ...
            return new PropertiesConfigSource(properties, path.getFileName().toString());
        } catch (IOException e) {
    ...
    public boolean isSupported(Path path) {
        return isFileExtension(path, ".properties");
    }
}
