...
You can create your own interceptor by implementing the
...
to intercept the resolution of a configuration key. You can use the `ConfigSourceInterceptorContext` to proceed the
interceptor chain. The chain can be short-circuited by returning your own instance of `ConfigValue`.

The `ConfigValue` objects holds information about the key name, value, config source origin and ordinal.

The Interceptor Chain is applied before any Conversion is performed on the Configuation.
...
If an expression cannot be expanded and no default is supplied a `NoSuchElementException` it thrown.

The `ExpressionConfigSourceInterceptor` is not registered by default. You need to register it via the ServiceLoader
mechanisn with your application.
